...
class TracksFragment(context: Context, attributeSet: AttributeSet) : MyViewPagerFragment(context, attributeSet) {
                    ...
                    TracksAdapter(
                        ...
                        activity.handleNotificationPermission { granted ->
                            if (granted) {
                                ...
                                prepareAndPlay(tracks, showPlayback = activity.config.showPlaybackActivity, "", startIndex = startIndex)
                            } else {
    ...
    override fun onSearchQueryChanged(text: String) {
        val filtered = tracks.filter { it.normalizeSearch(text, onlyTitleSearch = false) }.toMutableList() as ArrayList<Track>
